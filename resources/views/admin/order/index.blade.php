                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex fs-6">
                                            ...
                                            @elseif ($item['Status'] == 'Đang giao hàng' || $item['Status'] == 'Đã thanh toán')
                                                <div class="badge badge-sa-primary">{{ $item['Status'] }}</div>
                                    ...
                                    <td>
                                        <div class="sa-price"><span class="sa-price__symbol"></span><span
                                                ...
                                                ${{ number_format($item['Total Price']) }}</span><span
                                    ...
                                    <td class="d-flex gap-2">
                                        @if ($item['Status'] == 'Chờ xác nhận' )
                                            <form action="{{ route('admin.order.delivery', ['id' => $item['ID']]) }}">
                                                @csrf
                                                <input type="submit" value="Xác nhận & vận chuyển"
                                                    class="btn btn-success btn   btn-sm" type="button">
                                        ...
                                        <form action="{{ route('admin.order.show', ['id' => $item['ID']]) }}">
                                            @csrf
                                            <input type="submit" value="Detail" class="btn btn-primary btn   btn-sm"
                                                type="button">
                                        ...
                                        <form action="{{ route('admin.order.cancel') }} " method="POST">
                                            ...
                                            @if ($item['Status'] == 'Đã xác nhận' ||  $item['Status'] == 'Thành công' )
